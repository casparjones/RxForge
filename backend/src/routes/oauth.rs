...
pub async fn authorize(
    ...
    let client: Option<ClientRow> = sqlx::query_as(
        "SELECT id, redirect_uris FROM oauth_clients WHERE client_id = $1 AND active = true",
    )
    ...
    let result = sqlx::query(
        "INSERT INTO oauth_codes (id, client_id, code, redirect_uri, scope, expires_at, created_at)
    ...
    .bind(params.scope.as_deref().unwrap_or(""))
    .bind(expires_at)
    .execute(&state.db)
    .await;
...
async fn handle_auth_code(state: AppState, req: TokenRequest) -> Response {
    ...
    let code_row: Option<CodeRow> = sqlx::query_as(
        "SELECT id, client_id, redirect_uri, scope, used, expires_at
         FROM oauth_codes
         WHERE code = $1",
    )
    .bind(&code)
    ...
    let client_secret_str = req.client_secret.as_deref().unwrap_or("");
    ...
    let client: Option<ClientRow> = sqlx::query_as(
        "SELECT id, client_secret_hash FROM oauth_clients WHERE client_id = $1 AND id = $2",
    )
    .bind(client_id_str)
    ...
    let secret_valid = verify_client_secret(client_secret_str, &client.client_secret_hash);
    if !secret_valid {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({"error": "invalid_client"})),
